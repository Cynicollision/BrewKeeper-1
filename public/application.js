...
!function(){"use strict";angular.module("BrewKeeper",["ngResource","ngRoute"]).config(["$routeProvider","$locationProvider",function(a,b){var c={admin:{auth:function(a){return a.authorizeCurrentUserForRoute("admin")}},user:{auth:function(a){return a.authorizeAuthenticatedUserForRoute()}}};b.html5Mode({enabled:!0,requireBase:!1}),a.when("/",{templateUrl:"/partials/main/views/main",controller:"MainCtrl"}).when("/home",{templateUrl:"/partials/main/views/user-home",controller:"UserHomeCtrl"}).when("/signup",{templateUrl:"/partials/account/views/signup",controller:"SignupCtrl"}).when("/profile/edit",{templateUrl:"/partials/account/views/edit-profile",controller:"EditProfileCtrl",resolve:c.user}).when("/profile",{templateUrl:"/partials/account/views/view-profile",controller:"ViewProfileCtrl",resolve:c.user}).when("/brew",{templateUrl:"/partials/brew/views/brew-list",controller:"BrewListCtrl"}).when("/brew/add",{templateUrl:"/partials/brew/views/add-edit-brew",controller:"AddBrewCtrl"}).when("/brew/view/:id",{templateUrl:"/partials/brew/views/view-brew",controller:"ViewBrewCtrl"}).when("/brew/edit/:id",{templateUrl:"/partials/brew/views/add-edit-brew",controller:"EditBrewCtrl"}).when("/brew/delete/:id",{templateUrl:"/partials/brew/views/delete-brew",controller:"DeleteBrewCtrl"}).when("/recipe",{templateUrl:"/partials/recipe/views/recipe-list",controller:"RecipeListCtrl"}).when("/recipe/view/:id",{templateUrl:"/partials/recipe/views/view-recipe",controller:"ViewRecipeCtrl"}).when("/recipe/add",{templateUrl:"/partials/recipe/views/add-edit-recipe",controller:"AddRecipeCtrl"}).when("/recipe/edit/:id",{templateUrl:"/partials/recipe/views/add-edit-recipe",controller:"EditRecipeCtrl"}).when("/recipe/delete/:id",{templateUrl:"/partials/recipe/views/delete-recipe",controller:"DeleteRecipeCtrl"})}]),angular.module("BrewKeeper").run(["$rootScope","$location",function(a,b){a.$on("$routeChangeError",function(a,c,d,e){"not authorized"===e&&b.path("/")})}])}(),function(){"use strict";angular.module("BrewKeeper").controller("EditProfileCtrl",["$scope","$location","BaseCtrl","Auth","Identity","Notifier",function(a,b,c,d,e,f){function g(a,b){a.username=b.username,a.firstName=b.firstName,a.lastName=b.lastName}function h(){return{username:a.username,firstName:a.firstName,lastName:a.lastName}}a.submit=function(){var c=h();a.password&&a.password.length>0&&(c.password=a.password),d.updateCurrentUser(c).then(function(){f.notify("Your profile has been updated."),b.path("/")},function(a){f.error(a)})},c.init(function(){g(a,e.currentUser)})}])}(),function(){"use strict";angular.module("BrewKeeper").controller("SignupCtrl",["$scope","$location","Auth","Notifier",function(a,b,c,d){function e(){return{username:a.username,password:a.password,firstName:a.firstName,lastName:a.lastName}}a.signup=function(){var a=e();c.createUser(a).then(function(){c.authenticateUser(a.username,a.password).then(function(){d.notify("User account created!"),b.path("/")},function(a){d.error(a)})},function(a){d.error(a)}),$(".navbar-collapse")&&$(".navbar-collapse").collapse&&$(".navbar-collapse").collapse("hide")}}])}(),function(){"use strict";angular.module("BrewKeeper").controller("ViewProfileCtrl",["$scope","$q","BaseCtrl","Brew","Identity","Recipe",function(a,b,c,d,e,f){a.basicStats=[],a.mostBrewedRecipes=[],c.init(function(){var c=[],g=e.getCurrentUserId();b.all([d.getCountByUserId(g).then(function(a){c.push({name:"Brew count",value:a.data.count,displayOrder:1})}),d.getFirstBrewByDate(g).then(function(a){function b(a){return a.getMonth()+1+"/"+a.getDate()+"/"+a.getFullYear()}var d="No brews!";a.noBrews||(d=b(new Date(a.brewDate))),c.push({name:"Brewing since",value:d,displayOrder:3})}),f.getCountByUserId(g).then(function(a){c.push({name:"Recipe count",value:a.data.count,displayOrder:2})}),f.getTopBrewedRecipes(g).then(function(b){var d=b.data[0],e=d?d.name:"No recipes!";c.push({name:"Most Brewed Recipe",value:e,displayOrder:4}),a.mostBrewedRecipes=b.data})]).then(function(){c.sort(function(a,b){return a.displayOrder-b.displayOrder}),a.basicStats=c})})}])}(),function(){"use strict";angular.module("BrewKeeper").factory("Auth",["$q","BrewKeeperApi","Identity","User",function(a,b,c,d){return{authenticateUser:function(e,f){var g=a.defer(),h={username:e,password:f};return b.post("/login",h).then(function(a){if(a.data.success){var b=new d;angular.extend(b,a.data.user),c.currentUser=b,g.resolve(a.data)}else g.resolve(!1)}),g.promise},createUser:function(e){var f=a.defer(),g=new d(e);return b.post("/api/users/",e).then(function(){c.currentUser=g,f.resolve(g)},function(a){f.reject(a.data.reason)}),f.promise},updateCurrentUser:function(d){var e=a.defer(),f=angular.copy(c.currentUser);return angular.extend(f,d),b.put("/api/users/",d).then(function(){c.currentUser=f,e.resolve(!0)},function(a){e.reject(a.data.reason)}),e.promise},logoutUser:function(){var d=a.defer();return b.post("/logout",{logout:!0}).then(function(a){c.currentUser=void 0,d.resolve(!0)}),d.promise},authorizeCurrentUserForRoute:function(b){return c.isAuthorized(b)?!0:a.reject("not authorized")},authorizeAuthenticatedUserForRoute:function(){return c.isAuthenticated()?!0:a.reject("not authorized")}}}])}(),function(){"use strict";angular.module("BrewKeeper").service("Identity",["$window","User",function(a,b){this.currentUser=null,this.bootstrapCurrentUserFromWindow=function(){a.bkCurrentUser&&(this.currentUser=new b,angular.extend(this.currentUser,a.bkCurrentUser))},this.getCurrentUserId=function(){return this.currentUser?this.currentUser._id:-1},this.isAuthenticated=function(){return!!this.currentUser},this.isAuthorized=function(a){return!!this.currentUser&&this.currentUser.roles.indexOf(a)>-1},this.bootstrapCurrentUserFromWindow()}])}(),function(){"use strict";var a=angular.module("BrewKeeper");a.factory("User",["$resource",function(a){var b=a("/api/users/:id",{_id:"@id"});return b.prototype.isAdmin=function(){return this.roles&&this.roles.indexOf("admin")>-1},b}])}(),function(){"use strict";angular.module("BrewKeeper").controller("AddBrewCtrl",["$scope","$controller","BaseCtrl","Brew","BrewStatus","DatePicker","Identity","Notifier","Recipe",function(a,b,c,d,e,f,g,h,i){function j(){a.statuses&&(a.brewStatusCde=a.statuses[0]),a.brewBatchSize=1}b("BaseAddEditBrewCtrl",{$scope:a}),a.updateBrewName=function(){var b=a.brewRecipe._id,c=a.brewRecipe.name;i.getBrewCount(b).then(function(b){a.brewName=c+" #"+(b.data.count+1)},function(a){h.error(a)})},a.submitBrew=function(){var b=a.getFormBrewData();d.save(b).then(function(a){c.successRedirect("Brew added.","/brew/")},function(a){h.error(a)})},c.init(function(){a.recipes=[],a.hasRecipes=!1,a.brewUrl="/brew/",a.statuses=e.getStatuses(),j(),i.getByUserId(g.getCurrentUserId()).then(function(b){a.recipes=b.data.sort(function(a,b){return a.name>b.name}),a.hasRecipes=!!a.recipes.length,a.recipes&&a.recipes.length&&(a.brewRecipe=a.recipes[0],a.updateBrewName())})})}])}(),function(){"use strict";angular.module("BrewKeeper").controller("BaseAddEditBrewCtrl",["$scope","$location","Identity","Notifier",function(a,b,c,d){a.getFormBrewData=function(){return{id:a.brewId,name:a.brewName,batchSize:a.brewBatchSize,description:a.brewDescription,ownerId:c.getCurrentUserId(),recipeId:a.brewRecipe._id,statusCde:a.brewStatusCde?a.brewStatusCde.id:-1,brewDate:a.brewBrewDate,bottleDate:a.brewBottleDate,chillDate:a.brewChillDate}}}])}(),function(){"use strict";angular.module("BrewKeeper").controller("BrewListCtrl",["$scope","$location","BaseCtrl","Brew","BrewStatus","Identity",function(a,b,c,d,e,f){function g(){d.getCountByUserId(h).then(function(b){var c=b.data.count;a.hasBrews=!!c,c>a.listLimit?(a.limitResults=!0,d.getByUserId(h,a.listLimit).then(function(b){a.brews=b.data})):a.getAllCurrentUserBrews()})}var h=f.getCurrentUserId();a.brews=[],a.listLimit=10,a.limitResults=!1,a.hasBrews=!1,a.getStatusDisplay=e.getDisplay,a.predicate="brewDate",a.reverse=!0,a.getAllCurrentUserBrews=function(){a.limitResults=!1,d.getByUserId(h).then(function(b){a.brews=b.data})},a.doAddBrew=function(){b.path("/brew/add")},a.order=function(b){a.reverse=a.predicate===b?!a.reverse:!1,a.predicate=b},c.init(function(){g()})}])}(),function(){"use strict";angular.module("BrewKeeper").controller("DeleteBrewCtrl",["$scope","$routeParams","$location","BaseCtrl","Brew","Identity","Notifier",function(a,b,c,d,e,f,g){a.onConfirmDelete=function(){e.remove(b.id).then(function(a){g.notify("Brew deleted"),c.path("/brew")})},a.onCancelDelete=function(){c.path("/brew/view/"+b.id)},d.init(function(){a.brew=null,e.getByBrewId(b.id).then(function(b){e.isBrewOwnedByUser(b.data,f.getCurrentUserId())?a.brew=b.data:c.path("/")})})}])}(),function(){"use strict";angular.module("BrewKeeper").controller("EditBrewCtrl",["$scope","$q","$controller","$filter","$routeParams","BaseCtrl","Brew","BrewStatus","DatePicker","Identity","Notifier","Recipe",function(a,b,c,d,e,f,g,h,i,j,k,l){function m(b){a.brewId=b._id,a.brewName=b.name,a.brewBatchSize=b.batchSize,a.brewDescription=b.description,a.brewRecipe=b.recipeId,a.brewBrewDate=d("date")(b.brewDate,"M/d/yyyy"),a.brewBottleDate=d("date")(b.bottleDate,"M/d/yyyy"),a.brewChillDate=d("date")(b.chillDate,"M/d/yyyy"),a.brewUrl="/brew/view/"+a.brewId,a.brewStatusCde=a.statuses.filter(function(a){return a.id===b.statusCde})[0]}c("BaseAddEditBrewCtrl",{$scope:a}),a.submitBrew=function(){var b=a.getFormBrewData(),c="/brew/view/"+a.brewId;g.update(b).then(function(){f.successRedirect("Brew updated.",c)},function(a){k.error(a)})},f.init(function(){a.recipes=[],a.statuses=h.getStatuses(),b.all([g.getByBrewId(e.id).then(function(b){a.brew=b.data,m(b.data)}),l.getByUserId(j.getCurrentUserId()).then(function(b){a.recipes=b.data.sort(function(a,b){return a.name>b.name}),a.hasRecipes=!!a.recipes.length})]).then(function(){a.brewRecipe=a.recipes.filter(function(b){return a.brew&&b._id===a.brew.recipeId})[0]})})}])}(),function(){"use strict";angular.module("BrewKeeper").controller("ViewBrewCtrl",["$scope","$routeParams","$location","$window","BaseCtrl","Brew","BrewStatus","Identity","Notifier","Recipe",function(a,b,c,d,e,f,g,h,i,j){function k(b){j.getByRecipeId(b).then(function(b){a.recipeName=b.data.name},function(a){i.error(a)})}a.isBrewOwnedByCurrentUser=function(){return a.brew?f.isBrewOwnedByUser(a.brew,h.getCurrentUserId()):void 0},a.doEdit=function(){c.path("/brew/edit/"+a.brew._id)},a.doDelete=function(){c.path("/brew/delete/"+a.brew._id)},a.doGoBack=function(){c.path("/brew/")},e.init(function(){a.statusLookup=g,a.recipeName="",a.brew=null,f.getByBrewId(b.id).then(function(b){a.brew=b.data,k(a.brew.recipeId)},function(a){i.error(a),c.path("/brew")})})}])}(),function(){"use strict";var a=angular.module("BrewKeeper");a.factory("Brew",["$q","BrewKeeperApi","Identity",function(a,b,c){function d(c,d,e){var f=a.defer(),g="/api/brew/user/"+c;return d=d||0,g+="?limit="+d,e&&(g+="&active=1"),b.get(g).then(function(a){f.resolve(a)},function(a){f.reject(a.data.reason)}),f.promise}return{isBrewOwnedByUser:function(a,b){return a?b===a.ownerId:!1},getAll:function(){var c=a.defer();return b.get("/api/brew/").then(function(a){c.resolve(a)},function(a){c.reject(a.data.reason)}),c.promise},getByUserId:function(a,b){return d(a,b,!1)},getActiveByUserId:function(a){return d(a,-1,!0)},getCountByUserId:function(c){var d=a.defer(),e="/api/brew/user/count/"+c;return b.get(e).then(function(a){d.resolve(a)},function(a){d.reject(a)}),d.promise},getByBrewId:function(c){var d=a.defer(),e="/api/brew/"+c;return b.get(e).then(function(a){d.resolve(a)},function(a){d.reject(a.data.reason)}),d.promise},save:function(c){var d=a.defer();return b.post("/api/brew/",c).then(function(a){d.resolve(a)},function(a){d.reject(a.data.reason)}),d.promise},update:function(c){var d=a.defer();return b.put("/api/brew/",c).then(function(a){d.resolve(a)},function(a){d.reject(a.data.reason)}),d.promise},remove:function(c){var d=a.defer(),e="/api/brew/"+c;return b["delete"](e).then(function(a){d.resolve(a)},function(a){d.reject(a.data.reason)}),d.promise},getFirstBrewByDate:function(c){var d=a.defer(),e="/api/brew/user/first/"+c;return b.get(e).then(function(a){var b=""===a.data?{noBrews:!0}:a.data;d.resolve(b)},function(a){d.reject(a.data.reason)}),d.promise}}}])}(),function(){"use strict";angular.module("BrewKeeper").factory("BrewStatus",function(){var a=[{id:0,name:"Not started yet"},{id:1,name:"Fermenting"},{id:2,name:"Bottled"},{id:3,name:"Chilling"},{id:4,name:"Gone"}];return{getDisplay:function(b){return b>=0&&b<a.length?a[b].name:"Invalid brewStatusCde"},getStatuses:function(){return a},NotStartedYet:0,Fermenting:1,Bottled:2,Chilling:3,Gone:4}})}(),function(){"use strict";angular.module("BrewKeeper").factory("BaseCtrl",["$location","Identity","Notifier",function(a,b,c){function d(a){b.isAuthenticated()||f("Please log in.","/"),a&&a()}function e(b,d){c.notify(b),a.path(d)}function f(b,d){c.notify(b),a.path(d)}return{init:d,successRedirect:e,errorRedirect:f}}])}(),angular.module("BrewKeeper").factory("BrewKeeperApi",["$http",function(a){var b=function(a){var b=[];for(var c in a)b.push(encodeURIComponent(c)+"="+encodeURIComponent(a[c]));return b.join("&")};return{get:function(c){return a({method:"GET",isArray:!1,url:c,transformRequest:b,headers:{"Content-Type":"application/x-www-form-urlencoded"}})},post:function(c,d){return a({method:"POST",isArray:!1,url:c,data:d,transformRequest:b,headers:{"Content-Type":"application/x-www-form-urlencoded"}})},put:function(c,d){return a({method:"PUT",isArray:!1,url:c,data:d,transformRequest:b,headers:{"Content-Type":"application/x-www-form-urlencoded"}})},"delete":function(c){return a({method:"DELETE",isArray:!1,url:c,transformRequest:b,headers:{"Content-Type":"application/x-www-form-urlencoded"}})}}}]),function(){"use strict";angular.module("BrewKeeper").service("DatePicker",function(){$(".datepicker").datepicker&&$(".datepicker").datepicker({format:"m/d/yyyy",autoclose:!0,forceParse:!1})})}(),function(){"use strict";var a=angular.module("BrewKeeper");a.value("Toastr",toastr),a.service("Notifier",["Toastr",function(a){this.notify=function(b){a.success(b)},this.error=function(b){a.error(b)}}])}(),function(){"use strict";angular.module("BrewKeeper").controller("MainCtrl",["$scope","$location","Auth","Identity","Notifier",function(a,b,c,d,e){a.identity=d,a.signin=function(a,d){c.authenticateUser(a,d).then(function(a){a?b.path("/home"):e.error("Username/password incorrect.")}),$(".navbar-collapse")&&$(".navbar-collapse").collapse&&$(".navbar-collapse").collapse("hide")},d.isAuthenticated()&&b.path("/home")}])}(),function(){"use strict";angular.module("BrewKeeper").controller("NavBarLoginCtrl",["$scope","$location","Auth","Identity","Notifier",function(a,b,c,d,e){function f(){a.username="",a.password=""}a.identity=d,a.signout=function(){c.logoutUser().then(function(){f(),$(".navbar-collapse").collapse("hide"),e.notify("Logged out"),b.path("/")})}}])}(),function(){"use strict";angular.module("BrewKeeper").controller("UserHomeCtrl",["$scope","$location","BaseCtrl","Brew","BrewStatus","Identity",function(a,b,c,d,e,f){a.hasActivity=!0,a.fermentingBrews=[],a.bottledBrews=[],a.chillingBrews=[],c.init(function(){d.getActiveByUserId(f.getCurrentUserId()).then(function(b){var c=b.data;a.hasActivity=!!c.length,a.fermentingBrews=c.filter(function(a){return a.statusCde===e.Fermenting}),a.bottledBrews=c.filter(function(a){return a.statusCde===e.Bottled})})})}])}(),function(){"use strict";angular.module("BrewKeeper").controller("AddRecipeCtrl",["$scope","$location","Recipe","Identity","Notifier",function(a,b,c,d,e){function f(){return{ownerId:d.getCurrentUserId(),name:a.recipeName,description:a.recipeDescription,sourceName:a.recipeSourceName,sourceUrl:a.recipeSourceUrl}}a.recipeUrl="/recipe/",a.submitRecipe=function(){var a=f();c.save(a).then(function(a){e.notify("Recipe added!"),b.path("/recipe")},function(a){e.error(a)})}}])}(),function(){"use strict";angular.module("BrewKeeper").controller("DeleteRecipeCtrl",["$scope","$routeParams","$location","BaseCtrl","Identity","Recipe","Notifier",function(a,b,c,d,e,f,g){a.onConfirmDelete=function(){f.remove(b.id).then(function(b){g.notify('Recipe "'+a.recipe.name+'" deleted'),c.path("/recipe")})},a.onCancelDelete=function(){c.path("/recipe/view/"+b.id)},d.init(function(){a.recipe=null,f.getByRecipeId(b.id).then(function(b){f.isRecipeOwnedByUser(b.data),e.getCurrentUserId()?a.recipe=b.data:c.path("/")})})}])}(),function(){"use strict";angular.module("BrewKeeper").controller("EditRecipeCtrl",["$scope","$routeParams","$location","BaseCtrl","Identity","Recipe","Notifier",function(a,b,c,d,e,f,g){a.getFormRecipeData=function(){return{id:a.recipeId,ownerId:e.getCurrentUserId(),name:a.recipeName,description:a.recipeDescription,sourceName:a.recipeSourceName,sourceUrl:a.recipeSourceUrl}},a.submitRecipe=function(){var b=a.getFormRecipeData(),d="/recipe/view/"+a.recipeId;f.update(b).then(function(){g.notify("Recipe updated."),c.path(d)},function(a){g.error(a)})},d.init(function(){f.getByRecipeId(b.id).then(function(b){var c=b.data;a.recipeId=c._id,a.recipeName=c.name,a.recipeDescription=c.description,a.recipeSourceName=c.sourceName,a.recipeSourceUrl=c.sourceUrl,a.recipeUrl="/recipe/view/"+c._id})})}])}(),function(){"use strict";angular.module("BrewKeeper").controller("RecipeListCtrl",["$scope","$location","BaseCtrl","Identity","Recipe",function(a,b,c,d,e){function f(){e.getCountByUserId(g).then(function(b){var c=b.data.count;a.hasRecipes=!!c,c>a.listLimit?(a.limitResults=!0,e.getByUserId(g,a.listLimit).then(function(b){a.recipes=b.data})):a.getAllCurrentUserRecipes()})}var g=d.getCurrentUserId();a.recipes=[],a.listLimit=10,a.limitResults=!1,a.hasRecipes=!1,a.predicate="name",a.reverse=!1,a.getAllCurrentUserRecipes=function(){a.limitResults=!1,e.getByUserId(g).then(function(b){a.recipes=b.data})},a.doAddRecipe=function(){b.path("/recipe/add")},a.order=function(b){a.reverse=a.predicate===b?!a.reverse:!1,a.predicate=b},c.init(function(){f()})}])}(),function(){"use strict";angular.module("BrewKeeper").controller("ViewRecipeCtrl",["$scope","$q","$routeParams","$location","BaseCtrl","Identity","Recipe","Notifier",function(a,b,c,d,e,f,g,h){function i(a){return a.sourceUrl&&0!==a.sourceUrl.indexOf("http://")&&(a.sourceUrl="http://"+a.sourceUrl),a.sourceUrl}a.isRecipeOwnedByCurrentUser=function(){return a.recipe?g.isRecipeOwnedByUser(a.recipe,f.getCurrentUserId()):void 0},a.doEdit=function(){d.path("/recipe/edit/"+a.recipe._id)},a.doDelete=function(){d.path("/recipe/delete/"+a.recipe._id)},a.doGoBack=function(){d.path("/recipe/")},e.init(function(){b.all([g.getByRecipeId(c.id).then(function(b){var c=b.data;a.recipe=c,a.recipe.sourceUrl=i(c)},function(a){h.error(a),d.path("/recipe")}),g.getBrewCount(c.id).then(function(b){var c=b.data.count;a.recipe.timesBrewed=c})])})}])}(),function(){"use strict";var a=angular.module("BrewKeeper");a.factory("Recipe",["$q","BrewKeeperApi",function(a,b){return{isRecipeOwnedByUser:function(a,b){return a?b===a.ownerId:!1},getAll:function(){var c=a.defer();return b.get("/api/recipe/").then(function(a){c.resolve(a)},function(a){c.reject(a.data.reason)}),c.promise},getByUserId:function(c,d){var e=a.defer(),f="/api/recipe/user/"+c;return d=d||0,f+="?limit="+d,b.get(f).then(function(a){e.resolve(a)},function(a){e.reject(a.data.reason)}),e.promise},getCountByUserId:function(c){var d=a.defer(),e="/api/recipe/user/count/"+c;return b.get(e).then(function(a){d.resolve(a)},function(a){d.reject(a)}),d.promise},getByRecipeId:function(c){var d=a.defer(),e="/api/recipe/"+c;return b.get(e).then(function(a){d.resolve(a)},function(a){d.reject(a.data.reason)}),d.promise},save:function(c){var d=a.defer();return b.post("/api/recipe/",c).then(function(a){d.resolve(a)},function(a){d.reject(a.data.reason)}),d.promise},update:function(c){var d=a.defer();return b.put("/api/recipe/",c).then(function(a){d.resolve(a)},function(a){d.reject(a.data.reason)}),d.promise},remove:function(c){var d=a.defer(),e="/api/recipe/"+c;return b["delete"](e).then(function(a){d.resolve(a)},function(a){d.reject(a)}),d.promise},getBrewCount:function(c){var d=a.defer(),e="/api/recipe/count/"+c;return b.get(e).then(function(a){d.resolve(a)},function(a){d.reject(a)}),d.promise},getTopBrewedRecipes:function(c,d){var e=a.defer(),f="/api/recipe/user/top/"+c;return d=d||0,f+="?limit="+d,b.get(f).then(function(a){e.resolve(a)},function(a){e.resolve(a)}),e.promise}}}])}();